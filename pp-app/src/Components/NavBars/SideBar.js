...
export function SideBar(showSideBar, setShowSideBar, navigate, setToggleDropdown, toggleDropdown, logOut) {
    const {currentUser, profilePicture }= useCurrentAdmin(); 
    return <div  className={`bg-white rounded-br-md z-20 absolute h-fit min-w-64 w-fit top-20 left-0 transform transition-transform duration-700 ${
        ...
        <div className="h-fit z-20 px-3 py-4 overflow-y-auto bg-white border border-t-[#e7e6e6] rounded-br-md">
            ...
            <ul className="pt-4 mt-4 space-y-2 font-medium border-t border-dashed border-[#e7e6e6]">
                <li onClick={() => navigate("/settingsAdmin")}>
            ...
            <ul className="pt-4 mt-4 space-y-2 font-medium border-t border-dashed border-[#e7e6e6]">
                <li>
                    <a href="#" className="flex items-center p-3 text-gray-700 rounded-lg">
                        {(profilePicture==="null" || profilePicture===null || profilePicture===undefined) ? <></> : <img className="h-12 w-12 rounded-full" src={profilePicture} alt="avatar"></img>}
                        <span className="flex-1 ml-3 whitespace-nowrap font-Pop-L uppercase tracking-widest text-xs">{currentUser.name+" "+ currentUser.lastname}</span>
                    </a>
                </li>
